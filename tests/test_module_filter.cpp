...
int main()
{
    ...
    const bool network_seen = g_capture.find("network filtered") != std::string::npos;
    ...
    const bool trace_seen = g_capture.find("trace accepted", before) != std::string::npos;
    ...
    {
        std::fprintf(stderr,
                     "alloc_seen=%d network_seen=%d alloc_filtered_seen=%d trace_seen=%d\\n%s\\n",
